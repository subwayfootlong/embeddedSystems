...
void mq135_run_loop(void) {
    ...
    // Main Reading Loop - Starts reading immediately, regardless of warm-up state
    while (true) {
        
        // Read Sensor and Handle Errors
        status = mq135_read(&reading);
        
        if (status == MQ135_OK) {
            mq135_print(&reading);
        } else {
            // Failure: Print the specific error message
            printf("🛑 READ ERROR (%d): %s\n", status, mq135_strerror(status));
        }
        
        // Wait for next sample (1000ms)
        sleep_ms(1000); 
    }
}